...
class PaymentHandler(webapp2.RequestHandler):
    ...
    def get(self):
        ...
        ordr = db.GqlQuery("SELECT * FROM Orders_DB WHERE transaction_id = '%s'" %transaction_ID)
        ordr = ordr.get()
        if ordr:            
            params_new_post['user_address'] = ordr.delivery_address
            params_new_post['id_of_order'] = ordr.key()
            params_new_post['delivery_date'] = ordr.delivery_time
            order_description = ordr.order_details
            #self.response.out.write('salam')
            #self.response.out.write(order_description)
            order_details_html = ''
            items_lst = []
                        
            em = re.search('item_count=\S+', order_description)
            if em:
                temp = em.group(0)
                item_count = int(temp[11:len(temp)])
                #self.response.out.write(str(item_count))    
            ...
            for i in range(1,int(item_count)+1):
                ...
                em = re.search('%s\S+'%str_temp, order_description)
                if em:
                    temp = em.group(0)
                ...
                em = re.search('%s\d+'%str_temp, order_description)                
                if em:
                    temp = em.group(0)
                    this_item_count = temp[15:len(temp)]                
                ...
                if em:
                    temp = em.group(0)
                    delivery_date = temp[14:len(temp)]
            ...
            if re.search('^SUCCESS', response_str):
                ...
                for i in range(1,int(item_count)+1):
                    ...
                    #self.response.out.write(delivery_address)            
                    if userid:
                        b = Orders_DB(transaction_id = transaction_ID,price = payment_amount,chef_id = chef_id,order_status="confirmed",
